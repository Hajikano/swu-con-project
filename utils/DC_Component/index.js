...
DC_Component.prototype.getData = getData
...
DC_Component.prototype.setData = setData

function getDataPath(path) {
  return this._name + '.' + path
}
DC_Component.prototype.getDataPath = getDataPath
...
DC_Component.prototype.setStyle = setStyle
...
DC_Component.prototype.update = update
...
DC_Component.prototype.animation = animation

//样式设定方法

function boxSizing(mode = 'border-box') {
  this._style['_']['box-sizing'] = mode
  return this
}
DC_Component.prototype.boxSizing = boxSizing

function margin(postion = null, mode = 'auto') {
  this._style['_']['margin' + (postion ? `-${postion}` : '')] = mode
  return this
}
DC_Component.prototype.padding = padding

function padding(postion = null, mode = '10px') {
  this._style['_']['padding' + (postion ? `-${postion}` : '')] = mode
  return this
}
DC_Component.prototype.margin = margin
...
DC_Component.prototype.VStack = VStack
...
DC_Component.prototype.HStack = HStack

function shadow(blur = 5, h = '0px', v = '0px', spread = '1px', color = 'rgba(99, 99, 172, 0.2)', inset = false) {
  let colorSet = ColorSet
  this._style['_']['box-shadow'] = `${h} ${v} ${blur}px ${spread} ${colorSet[color] ? colorSet[color] : color} ${
    inset ? inset : ''
  }`
  return this
}
DC_Component.prototype.shadow = shadow
...
DC_Component.prototype.radius = radius
...
DC_Component.prototype.backgroundColor = backgroundColor
...
DC_Component.prototype.frame = frame
...
DC_Component.prototype.fontSize = fontSize

function fontBold(type = 'bold') {
  this._style['_']['font-weight'] = type
  return this
}
DC_Component.prototype.fontBold = fontBold
...
DC_Component.prototype.fontColor = fontColor

function fontAlign(align = 'center') {
  this._style['_']['text-align'] = align
  return this
}
DC_Component.prototype.fontAlign = fontAlign
...
DC_Component.prototype.offset = offset

function overflow(mode = 'scroll') {
  this._style['_']['overflow'] = mode
  return this
}
DC_Component.prototype.overflow = overflow

function blur(radius = 5) {
  this._style['_']['filter'] = `blur(${radius}px)`
  return this
}
DC_Component.prototype.blur = blur

module.exports = { DC_Component, SizeMap, ColorSet }
